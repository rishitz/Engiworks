...
//                    session.setAttribute("AdminName", getUserName());
                    return "/Admin/AdminL.xhtml?faces-redirect=true";
                }
    ...
    public void logout()
    {
        ...
        try {
            if(session.getAttribute("AdminName") != null || session.getAttribute("StudentName") != null || session.getAttribute("CollegeName") != null)
            {
                session.invalidate();
                req.logout();
