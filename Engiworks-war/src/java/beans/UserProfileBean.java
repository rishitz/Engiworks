...
//        address= u.getAddress();
    ...
    public Collection<Tblrequirement> userPostJob()
    {
      Collection<Tblrequirement> r=userbean.userPostJob();
        
//         HttpServletRequest req = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest();
//        HttpSession session=req.getSession(false);
//       int u1= (int)session.getAttribute("userId");
//         Response response=rc.getuserJob(Response.class,u1+"");
//         ArrayList<Tblrequirement> alist=new ArrayList<Tblrequirement>();
//         GenericType<Collection<Tblrequirement>> gAdd = new GenericType<Collection<Tblrequirement>>(){};
//         alist = (ArrayList<Tblrequirement>) response.readEntity(gAdd);
       return r;
    }
